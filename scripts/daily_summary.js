...
const { buildErrorReasonReport } = require('./error_reason_advisor');
...
function buildSummaryMessage() {
  ...
  // 错因分析（近7天），优先于模块建议——error_reason 才是指导「怎么改」的信号
  const errorReport = buildErrorReasonReport(7);
  // 模块建议作为回退（错因数据不足时用）
  const moduleSuggestion = buildSuggestion(weakestToday, cache.weak_modules);

  const parts = [`今日总结：${modLine}`];
  if (errorReport) {
    parts.push(errorReport);
  } else if (moduleSuggestion) {
    parts.push(moduleSuggestion);
  }
