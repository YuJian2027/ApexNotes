...
const { locateAll }  = require('./link_questions');
...
function runPython(args, timeout = 60_000) {
  return new Promise((resolve, reject) => {
    ...
    const candidates = [
      venvPython,
      path.join(os.homedir(), '.workbuddy', 'binaries', 'python', 'envs', 'default', 'bin', 'python3'),
      'python3',
      'python',
    ];
    let idx = 0;
    ...
    function attempt() {
      if (idx >= candidates.length) {
        return reject({ error: new Error('未找到可用的 Python（需要 openpyxl）'), stderr: '' });
      }
...
headers = ["日期","科目","题型","知识路径","错误原因","题目内容","视觉描述","正确答案","解析/批注","知识点标签","状态","来源","截图"]
col_widths = [10, 12, 18, 35, 12, 40, 45, 8, 30, 20, 10, 12, 20]
...
for ri, row in enumerate(wrong_rows, 2):
    for ci, val in enumerate(row, 1):
        ...
        cell.alignment = Alignment(vertical="center", wrap_text=(ci in [4, 6, 7, 9]))
...
async function exportXlsx({ pendingOnly = false, moduleFilter = null, daysFilter = null, withImages = true } = {}) {
  ...
  // ── 自动定位：为每道错题匹配知识框架路径 ───────────────────────
  const locations = locateAll(filtered);
  const pathMap = {};
  locations.forEach(loc => {
    if (loc.question_id && loc.confidence.level !== 'none') {
      pathMap[loc.question_id] = loc.path;
    }
  });
