...
function parseQuickEntry(text) {
  // 支持两种写法：
  //   ① 纯标记：资料-增长率-公式不熟
  //   ② 标记 + 换行 + 题干：资料-增长率-公式不熟\n某省2025年...A.10% B.12%
  const lines = text.split(/\n+/).map(l => l.trim()).filter(l => l);
  if (!lines.length) return null;

  const head = lines[0];                       // 第一行 = 快捷标记
  ...
  return {
    ...
    // 有题干用题干（可复习），无题干回退到原标记串（保持兼容）
    question_text: body || head,
    error_reason,
    keywords,
    status,
...
/**
 * 多行快捷批量录入
 * 每行一道错题，格式同 parseQuickEntry：
 *   判断-逻辑判断-粗心
 *   资料-增长率-公式不熟
 *   言语-主旨-时间不够
 *
 * @returns {object|null} { source:'quick-batch', questions:[] }，null 表示不是多行快捷格式
 */
function parseQuickEntryBatch(text) {
  const lines = text.trim().split(/\n+/).map(l => l.trim()).filter(l => l);
  if (lines.length < 2) return null;  // 单行不算 batch

  // 按「快接头行」分块：每个 head 行开启新 block，后续非 head 行并入该 block 的题干。
  // 这样「标记\n题干」算一条，多条快捷可各有题干。
  const blocks = [];
  let current = null;
  for (const line of lines) {
    const firstSeg = line.split(/[-—·\/]/)[0].trim();
    if (normalizeModule(firstSeg)) {
      // 快接头行：开启新 block
      if (current) blocks.push(current);
      current = [line];
    } else if (current) {
      // 非 head 行：并入当前 block 的题干
      current.push(line);
    }
    // 开头连续的非 head 行直接丢弃（无归属）
  }
  if (current) blocks.push(current);

  const entries = blocks
    .map(b => parseQuickEntry(b.join('\n')))
    .filter(e => e !== null);

  // 至少 2 个有效 block 才算批量；单行带题干会落到这里（entries=1）退回 parseQuickEntry
...
function parseStudyInput(message) {
  // 优先尝试多行快捷批量（每行一道错题）
  const batch = parseQuickEntryBatch(message);
  if (batch) return batch;

  // 单行快捷录入格式：资料-乘积增长-公式不熟-待二刷
...
module.exports = { handleMessage, parseStudyInput, parseImageInput, parseQuickEntry, parseQuickEntryBatch, normalizeModule, detectMood };
